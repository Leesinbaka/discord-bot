...
import random
diu = []
#login method credit by GitHub:UndergroundLabs 
#thanks man
...
if __name__ == "__main__":
    ...
    if user_id:
        ...
        haha = session.get('https://www.facebook.com/groups/||GROUP NUMBER||/')#change here
        link = re.findall('data-ploi="(.+?)"',str(haha.text))#get image link 
        #group them up in a array(sorry for my grammar)
        for i in range(0,len(link)):
            qq = str(link[i]).replace('amp;','')
            diu.append(qq)
        print(len(link))
    else:
        print ('Login Failed')
